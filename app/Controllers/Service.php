...
class Service extends BaseController
{
    #------------------------------------    
    # Author: Taj Tech Ltd
    # Author link: https://www.tajtechltd.com
    # Dynamic style php file
    # Developed by :TajTechLtd Team
    #------------------------------------    
    public function __construct()
    {
        helper('form');
    }
    public function tajtech_service_type_form()
    {
    ...
    public function tajtech_service_form1()
    {
        ...
        $data['allData'] =  $service_model->getData();
        return  view('service/service_form', $data);
    }
//save_service_type_data 
    public function save_service_type_data()
    {
        ...
        if ($this->request->getMethod() == "post") {
                ...
                $service_model = new ServiceModel();
                $response =  $service_model->add_service_type($myData);
                if ($response) {
                    $page_session->setTempdata('success', 'service type name save successfully.', 2);
                    return redirect()->to(current_url());
                } else {
                    $page_session->setTempdata('failed', 'service type name save failed.', 2);
                    return redirect()->to(current_url());
                }
        } else {
            echo view('service/service_type_form', $data);
        }
    }
    ...
    public function tajtech_CheckserviceList()
    {
    ...
    public function tajtech_service_form($id = null)
    {
      ...
      //  $num_column  = count($tablecolumn) - 5;
        ...
        //eturn $this->template->layout($data);
    }
    ...
    public function tajtech_service_invoice_form()
    {
        if ($this->request->getMethod() == 'post') {
            $rules = [
    ...
    public function tajtech_Checkservice_invoiceList()
    {
        $postData = $this->request->getVar();
